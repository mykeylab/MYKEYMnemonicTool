...
func SupplyZeroForByte(length int, str []byte) []byte {
	if len(str) < length {
		strLength := len(str)
		newRest := make([]byte, 0)
		for i := 0; i < length-strLength; i++ {
			newRest = append(newRest, []byte{0}...)
		}
		str = append(newRest, str...)
	}
	return str
}
...
func compress(x, y *big.Int) []byte {
	...
	rest := x.Bytes()
	if len(rest) < 32 {
		rest = SupplyZeroForByte(32, rest)
	}
...
// 如果有异常不返回，仅返回空数组
func copyByte(source []byte) ([]byte) {
	if source == nil {
		log.Println("in copyByte source is nil.")
		return make([]byte, 0)
	}
